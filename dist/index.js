...
const writeYaml = (outputPath, obj) => {
  return pify_1.default(fs_1.default.writeFile)(
    `${outputPath}.yml`,
    js_yaml_1.default.safeDump(obj),
    'utf8'
  )
}
...
function loadLocaleFiles(locales, buildDir, ext) {
  ...
  for (const locale of locales) {
    ...
    try {
      const output = isJson(ext)
        ? JSON.stringify({})
        : js_yaml_1.default.safeDump({})
    ...
    let messages = isJson(ext)
      ...
      : js_yaml_1.default.safeLoad(fs_1.default.readFileSync(file, 'utf8'), {
          json: true
        })
...
exports.extractReactIntl = extract_react_intl_1.default
...
const extractMessage = async (
  ...
  const extractorOptions = Object.assign(
    {
      defaultLocale,
  ...
  const newLocaleMaps = await extract_react_intl_1.default(
    locales,
    pattern,
    extractorOptions,
    fillLocalesWithDefaultMessage
      ...
      let localeMap =
        locale === defaultLocale
          ? // Create a clone so we can use only current valid messages below
            Object.assign(
              Object.assign({}, oldLocaleMaps[locale]),
              newLocaleMaps[locale]
            )
          : Object.assign(
              Object.assign({}, newLocaleMaps[locale]),
              oldLocaleMaps[locale]
            )
      // Only keep existing keys
      localeMap = lodash_pick_1.default(
        localeMap,
        Object.keys(newLocaleMaps[locale])
      )
